# copied from: https://github.com/snu-mllab/GuidedQuant/blob/main/any_precision/evaluate/eval.py
...
def auto_model_load(
    ...
    if is_fp16:
        ...
        model = AnyBCQForCausalLM.from_quantized(model_path,
                    new_vocab_size=new_vocab_size).to(device)
...
def run_lm_eval(tokenizer, model, tasks, num_fewshot, verbose=True):
    ...
    if 'gsm8k_cot' in tasks:
        model_lm = lm_eval.models.huggingface.HFLM(pretrained=model, tokenizer=tokenizer, add_bos_token=True)
        print("=================Adding BOS token for GSM8K_COT=================")
    else:
        model_lm = lm_eval.models.huggingface.HFLM(pretrained=model, tokenizer=tokenizer)
    eval_results = lm_eval.simple_evaluate(model=model_lm, tasks=tasks, num_fewshot=num_fewshot, batch_size="auto")
