# copied from: https://github.com/Vahe1994/AQLM/blob/main/convert_to_hf.py
...
try:
    ...
    safetensors = None
import codegemm.utils.swap_linear as swap_linear
...
def get_converted_state_dict(model, config, nbits: int, in_path: os.PathLike) -> [dict, list[str]]:
    state_dict = {}
    ...
    for i in trange(num_layers):
        layer = torch.load(os.path.join(in_path, f"{i}.pth"), weights_only=True)
...
def get_metadata(in_path: os.PathLike) -> dict:
    ...
    return {
        ...
        "scale_group_size": quant_args["scale_group_size"],
    }
...
if __name__ == "__main__":
    ...
    model = AutoModelForCausalLM.from_pretrained(
        args.model, trust_remote_code=args.trust_remote_code, low_cpu_mem_usage=True, torch_dtype=torch.float16, cache_dir=args.cache_dir
    )

    if args.swap_in_place:
        # Swap nn.Linear with CodeGEMMLinear in place
        print("Swapping nn.Linear layers with CodeGEMMLinear...")
        model = swap_linear.swap_quant_model(
            model=model,
            config=old_config,
        ...
        print(f"Model with CodeGEMMLinear saved to {args.out_path}")

        # Add codegemm configuration to config.json
        config_path = os.path.join(args.out_path, "config.json")
        with open(config_path, "r") as f:
            config_dict = json.load(f)
        
        # Get model type to determine arch_config
        model_type = old_config.model_type
        if model_type in ["llama", "mistral", "mixtral", "gemma", "phi3", "qwen2"]:
            layers_name = "layers"
            model_name = "model"
            module_names = [
                "self_attn.q_proj",
        ...
        # Add codegemm configuration
        config_dict["codegemm"] = {
            "arch_config": {
        ...
        print(f"Added codegemm configuration to config.json")
    else:
        # Original state_dict conversion method
        state_dict, linear_weights_not_to_quantize = get_converted_state_dict(
            model, old_config, metadata["nbits_per_codebook"], args.in_path
        )
        ...
        if args.save_safetensors:
            assert safetensors
            model = AutoModelForCausalLM.from_pretrained(args.out_path, trust_remote_code=True, torch_dtype=torch.float16, cache_dir=args.cache_dir)
