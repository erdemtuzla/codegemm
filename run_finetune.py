# copied from: https://github.com/Vahe1994/AQLM/blob/main/finetune.py
...
from codegemm.quantization.convert_legacy_model_format import load_quantized_model_with_old_pickle
from codegemm.quantization.aq import QuantizedWeight
from codegemm.quantization.configurable_adam import ConfigurableAdamW
from codegemm.quantization.datautils import evaluate_perplexity, get_loaders, group_texts, split_long_texts
from codegemm.quantization.memory_efficient_loss import compute_kl_divergence_loss_values
from codegemm.quantization.modelutils import get_model, is_model_for_causal_lm
from codegemm.quantization.pv_optimizer import StraightThroughAdamW
from codegemm.quantization.pv_utils import (
...
def compute_validation_perplexities(args: argparse.Namespace, model: nn.Module, eval_datasets: dict):
    ...
    for dataset_name, eval_dataset in eval_datasets.items():
        ...
        if rank == 0:
            os.makedirs(args.save, exist_ok=True)
            log_path = os.path.join(args.save, "evaluation_log.txt")
            with open(log_path, "a") as log_file:
                log_file.write(f"{dataset_name} perplexity: {ppl:.4f}\n")
...
def main():
    ...
    tokenizer = transformers.AutoTokenizer.from_pretrained(args.base_model, cache_dir=args.cache_dir)
