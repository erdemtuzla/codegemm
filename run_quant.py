# copied from: https://github.com/Vahe1994/AQLM/blob/main/main.py
...
from codegemm.quantization.aq_engine import AQEngine
from codegemm.quantization.aq import QuantizedLinear
from codegemm.quantization.datautils import get_loaders
from codegemm.quantization.finetune import finetune_groupwise
from codegemm.quantization.modelutils import (
    FALCON_TYPES,
    find_sublayers,
    get_layers,
    get_lm_logits,
...
def perplexity_eval(model: PreTrainedModel, testenc: torch.LongTensor, args: Namespace) -> float:
    ...
    with open(log_path, "a") as log_file:
        log_file.write(f"{args.dataset_name} perplexity: {ppl:.4f}\n")
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Cache directory for the model and dataset.",
    )
    parser.add_argument(
        "--nsamples",
        type=int,
    ...
    parser.add_argument(
        ...
        type=int,
        default=128,
        help="How many input features are quantized together (scale factor)",
    )
    parser.add_argument(
        "--scale_nbits",
        type=int,
